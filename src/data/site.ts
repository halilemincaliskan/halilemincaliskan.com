...
export const build = {
  ...
  screen: {
    app: 'Verify',
    tabs: ['Build', 'Pipeline', 'Checks', 'History'],
    hero: { title: 'verify.sh is ready', body: 'Scroll to run the build.' },
    work: {
      title: 'BuyBuddy',
      subtitle: 'Simulator build',
      packagesHeader: 'Swift packages',
      packages: ['DesignSystem', 'Networking', 'Hardware'],
    ...
    workflow: {
      title: 'Pipeline',
      subtitle: 'verify.sh · every change',
      rows: ['Spec', 'Old code', 'Agents', 'Gate', 'Device', 'Log'],
      states: { wait: 'Waiting', run: 'Running', pass: 'Passed' },
    },
    gate: {
      title: 'Checks',
      subtitle: 'generic/platform=iOS',
    ...
    defects: {
      title: '42 defects',
      subtitle: 'Found after every check was green',
      items: ['İ ≠ I', 'Tag ID case', 'Pairing status', 'Retry state'],
      states: { open: 'Open', fixed: 'Fixed' },
    },
    experience: { title: 'History', subtitle: '2022 → now', now: 'Now' },
    contact: { title: 'Build Succeeded', subtitle: 'Ready to ship', action: 'Email Halil' },
  },
} as const;
